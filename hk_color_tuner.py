...
def load_data():
    ...
    conn.close()
    return [get_color(r["special_number"]) for r in rows]
...
class MarkovN:
    ...
    def train(self, seq, decay=1.0):
        ...
        for i in range(len(seq)-self.order-1, -1, -1):
            ...
            self.counts[ctx][nxt] += w
            self.total[ctx] += w
            w *= decay

    def predict(self, ctx):
        if not ctx:
            return {s: 1.0/3 for s in STATES}
...
class Ensemble:
    def __init__(self, temp=1.0, markov_order=2):
        self.temp = temp
        self.markov = MarkovN(markov_order)
        self.markov2 = MarkovN(max(1, markov_order-1))
...
def entropy_decomp(sub_preds_list, fused_list):
    ...
    for fused, subs in zip(fused_list, sub_preds_list):
        ...
        exp_ent += e/len(subs)
    total_ent /= len(fused_list)
    exp_ent /= len(fused_list)
...
def main():
    ...
    calib_seq = seq[-test_len-50:-test_len]  # 用于温度选择的额外50期
    ...
    for temp in [0.6, 0.8, 1.0, 1.2, 1.5]:
        ...
        for i in range(len(calib_seq)-1):
            recent = train_seq[-(i+4):] if i+4 <= len(train_seq) else train_seq
    ...
    # 最终模型
    model = Ensemble(temp=best_temp)
    model.train(train_seq)

    # 测试
    test_probs_fused, test_probs_sub, test_actuals = [], [], []
    uniform_loss = -math.log(1/3)
    delta_logloss = []

    for i in range(test_len-1):
        recent = train_seq[-(test_len-i):] if test_len-i > 0 else []
        fused, subs = model.predict(recent)
    ...
    avg_delta = np.mean(delta_logloss)
    ...
    with open('tuning_report.txt', 'w', encoding='utf-8') as f:
        f.write(f"最佳温度: {best_temp}\n校准集 ECE: {best_ece:.4f}\n测试集 ECE: {ece_final:.4f}\n")
        f.write(f"ΔLogLoss: {avg_delta:+.4f}\n p-value: {p_val:.4f}\nMI: {mi:.4f}\n")
