#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# 三彩种属性预测 V7.1 (修复校准 + 正确概率评估 + 贝叶斯概率更新)
...
# ========== Online Bayesian Updating (概率质量版) ==========
class OnlineBayesianWeight:
    ...
    def update(self, model: str, prob_actual: float):
        """
        使用预测概率质量更新：alpha += prob_actual, beta += (1 - prob_actual)
        """
...
class FeatureConditionalModel:
    def __init__(self, states: List[str], feature_name: str, n_bins: int = None):
        self.states = states
    ...
    def predict_next_probs(self, obs_seq: List[str]) -> Dict[str, float]:
        ...
        for t in range(1, T):
            for j in range(self.n_states):
                max_val = np.max(log_alpha[t-1] + np.log(self.A[:, j]))
                log_alpha[t, j] = max_val + np.log(np.sum(np.exp(log_alpha[t-1] + np.log(self.A[:, j]) - max_val)))
        probs = np.log(np.sum(np.exp(log_alpha[-1][:, None] + np.log(self.A) + np.log(self.B)), axis=0))
...
def expected_calibration_error(probs_list: List[float], outcomes_list: List[int], n_bins: int = 10) -> float:
    """probs_list: 每个预测的最大概率（或预测的概率）; outcomes_list: 1 if correct else 0"""
    if len(probs_list) == 0:
        return 0.0
...
class AttributeEngineV7_1:
    def __init__(self, name: str, order: int = 3, alpha: float = 1.0,
        ...
        # 特征模型
        self.tail_model = FeatureConditionalModel(self.states, "tail")
        self.mod7_model = FeatureConditionalModel(self.states, "mod7")
        self.cross_dist_model = FeatureConditionalModel(self.states, "cross_dist")
        # 在线贝叶斯权重（概率质量更新）
        ...
        # 历史特征值
        self.feature_history = {"tail": [], "mod7": [], "cross_dist": []}
    ...
    def predict_proba(self, recent_seq: List[str], recent_draws: List[Dict]) -> Dict[str, float]:
        ...
        # HMM
        hmm_probs = self.hmm.predict_next_probs(recent_seq) if self.use_hmm and self.hmm else {s:1/len(self.states) for s in self.states}
        # 特征模型
    ...
    def update_feedback(self, predicted_probs: Dict[str, float], actual: str):
        ...
        # 为每个模型更新相同的 reward（简化：所有模型共享同一 reward，或可分开）
        # 这里使用同一 reward，各模型独立更新
        for m in self.model_names:
...
# ========== 系统集成 V7.1 ==========
class PredictionSystemV7_1:
    def __init__(self, order: int = 3, min_ig: float = 0.01, temperature: float = 1.0, use_hmm: bool = True):
        self.order = order
        self.min_ig = min_ig          # 出手阈值（复用）
        self.temperature = temperature
        self.use_hmm = use_hmm
        self.engines = {
            "color": AttributeEngineV7_1("color", order, use_hmm=use_hmm, temperature=temperature),
            "size": AttributeEngineV7_1("size", order, use_hmm=use_hmm, temperature=temperature),
            "odd_even": AttributeEngineV7_1("odd_even", order, use_hmm=use_hmm, temperature=temperature)
        }
    ...
    def walk_forward_backtest(self, seqs: Dict[str, List[str]], draws: Dict[str, List[Dict]],
                              test_len: int = 100) -> Tuple[Dict[str, float], Dict[str, float], float, float, float, Dict[str, float]]:
        """
        返回: (准确率, LogLoss, 波色二中一, p_value, 平均KL散度, 各属性ECE)
        """
        total = 0
        ...
        actuals_list = []
        preds_list = []
        max_probs_list = {name: [] for name in self.engines}   # 用于ECE
        outcomes_list = {name: [] for name in self.engines}    # 1 if correct else 0
        ...
        for idx in range(start_idx, len(seqs["color"]) - 1):
            # 完全重新训练，无未来泄漏
            system = PredictionSystemV7_1(order=self.order, min_ig=self.min_ig,
            ...
            if pred["meta"]["should_act"]:
                ...
                for name in self.engines:
                    ...
                    if pred[name]["best_state"] == actuals[name]:
                        correct[name] += 1
                    # 记录 ECE 数据
                    maxp = pred[name]["max_prob"]
                    outcome = 1 if pred[name]["best_state"] == actuals[name] else 0
                    max_probs_list[name].append(maxp)
                    outcomes_list[name].append(outcome)
                # 波色二中一
                if pred["color"]["best_state"] == actuals["color"] or pred["color"]["second_state"] == actuals["color"]:
                    color_second_correct += 1
                actuals_list.append(actuals["color"])  # 任意一个
                preds_list.append(pred["color"]["best_state"])
                total += 1
        if total == 0:
            return ({name:0.0 for name in self.engines},
                    {name:0.0 for name in self.engines}, 0.0, 1.0, 0.0,
                    {name:0.0 for name in self.engines})
        # 准确率
        ...
        ece = {}
        for name in self.engines:
            ece[name] = expected_calibration_error(max_probs_list[name], outcomes_list[name])
        # 波色二中一
...
def print_dashboard(conn, lottery_name: str, order=3, min_ig=0.01, temperature=1.0,
    ...
    system = PredictionSystemV7_1(order=order, min_ig=min_ig, temperature=temperature, use_hmm=use_hmm)
...
def main():
    p = argparse.ArgumentParser(description="三彩种属性预测 V7.1 (正确概率评估)")
