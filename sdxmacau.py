#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# 三彩种属性预测 (老澳门/香港/新澳门) - 无子命令版本
# 直接运行: python sdxmacau.py [--order 3] [--min-conf 0.55] [--temp 1.0] [--backtest 30]
...
def process_lottery(lottery_name: str, args):
    ...
    try:
        ...
        system = PredictionSystem(order=args.order, min_confidence=args.min_conf, temperature=args.temp)
        system.train_all(seqs)
        recents = {name: seq[-args.order:] for name, seq in seqs.items()}
        pred = system.predict_all(recents)

        print(f"\n🔮 下一期属性预测 (阶数={args.order}, 温度={args.temp})")
        ...
        acc, logloss, color_second_acc = system.walk_forward_backtest(seqs, test_len=args.backtest)
        for name in acc:
...
def main():
    ...
    p.add_argument("--min-conf", type=float, default=0.55, help="出手最小平均概率阈值 (默认0.55)")
