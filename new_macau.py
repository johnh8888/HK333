...
def review_issue(conn, issue_no):
    ...
    for run in runs:
        ...
        pool14 = json.loads(pool14_row[0]) if pool14_row else mains
        pool20_row = conn.execute("SELECT numbers_json FROM prediction_pools WHERE run_id=? AND pool_size=20", (run_id,)).fetchone()
        pool20 = json.loads(pool20_row[0]) if pool20_row else mains
